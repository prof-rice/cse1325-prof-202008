...
void Controller::cli() {
    ...
    if (_grid.player_is_alive() && cmd != '0') _view.trophy(); else _view.explosion();
    _view.print_grid();
};

bool Controller::execute_cmd(char cmd) {
    ...
    if (cmd == '1') {_grid.move_player(down_left ); return true; }
    if (cmd == '2') {_grid.move_player(down      ); return true; }
    if (cmd == '3') {_grid.move_player(down_right); return true; }
    if (cmd == '4') {_grid.move_player(left      ); return true; }
    if (cmd == '5') {_grid.move_player(stay      ); return true; }
    if (cmd == '6') {_grid.move_player(right     ); return true; }
    if (cmd == '7') {_grid.move_player(up_left   ); return true; }
    if (cmd == '8') {_grid.move_player(up        ); return true; }
    if (cmd == '9') {_grid.move_player(up_right  ); return true; }
    if (cmd == '.') {_grid.teleport(); return true; }
    if (cmd == '?') {_view.print_help(); }
    return false;
};
